...
/**
 * Class pocketlistsAutomationRuleAbstract
 *
 * Класс описывает условия правила, которые должны сработать, чтобы выполнилось действие
 */
abstract class pocketlistsAutomationRuleAbstract implements pocketlistsAutomationRuleInterface, pocketlistsHtmlEditableInterface, pocketlistsSerializableInterface
{
    /**
     * @var pocketlistsAutomationRuleInterface
     */
    protected $nextRule;
    ...
    /**
     * @param pocketlistsAutomationRuleInterface $rule
     *
     * @return pocketlistsAutomationRuleInterface
     */
    public function setNext(pocketlistsAutomationRuleInterface $rule)
    {
        $this->nextRule = $rule;
     ...
     *
     * @return pocketlistsAutomationRuleAbstract
     */
    ...
    protected function getSelectCompareControl($selected = null, $types = [])
    {
        ...
        foreach ($types ?: pocketlistsComparision::getTypes() as $item) {
            $options[] = [
                'title' => _wp($item),
                'value' => $item,
            ];
    ...
    protected function fetchTemplate(array $data = [], $mode = 'view')
    {
        ...
        return $view->fetch(
            wa()->getAppPath(
                sprintf(
                    'templates/actions%s/automation/rule/%s.%s.html',
